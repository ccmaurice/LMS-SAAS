...
import {
  MAX_ORG_HERO_BYTES,
  isSafeOrgCertificateSealStoredValue,
  orgCertificateSealSettingPath,
...
async function unlinkOrgSealVariants(root: string, orgId: string) {
  const dir = path.join(root, "orgs", orgId);
  try {
    const names = await fs.readdir(dir);
    for (const name of names) {
      if (name.startsWith("seal.") && /\.(jpe?g|png|webp|gif)$/i.test(name)) {
...
export async function POST(req: Request) {
  ...
  const prev = parseOrganizationSettings(currentRow?.organizationSettings).certificateSealImageUrl?.trim();
  if (prev && isSafeOrgCertificateSealStoredValue(prev, orgId)) {
    await removeUpload(prev);
  }
  if (!blobStorageEnabled()) {
    await unlinkOrgSealVariants(getUploadRoot(), orgId);
  }

  const relKey = orgCertificateSealSettingPath(orgId, ext);
  ...
  const merged = mergeOrganizationSettings(currentRow?.organizationSettings, {
    certificateSealImageUrl: storedRef,
  });
  ...
  return NextResponse.json({
    certificateSealImageUrl: storedRef,
    previewUrl: `/api/public/organizations/${user.organization.slug}/certificate-seal`,
  });
}
