...
export async function POST(req: Request, ctx: { params: Promise<{ departmentId: string }> }) {
  ...
  const u = resolvedId
    ...
    : null;
  if (!u) return NextResponse.json({ error: "User must be a student in this school (check email)" }, { status: 400 });

  await prisma.studentDepartmentAffiliation.upsert({
    where: { departmentId_userId: { departmentId, userId: u.id } },
    create: {
      departmentId,
      userId: u.id,
      isPrimary: parsed.data.isPrimary ?? true,
    },
    update: { isPrimary: parsed.data.isPrimary ?? true },
  });
...
export async function DELETE(req: Request, ctx: { params: Promise<{ departmentId: string }> }) {
  ...
  await prisma.studentDepartmentAffiliation.deleteMany({ where: { departmentId, userId } });
  return NextResponse.json({ ok: true });
}
