...
export async function POST(req: Request, ctx: { params: Promise<{ departmentId: string }> }) {
  ...
  const u = resolvedId
    ...
    : null;
  if (!u) return NextResponse.json({ error: "User must be teacher or admin in this school (check email)" }, { status: 400 });

  await prisma.departmentInstructor.upsert({
    where: { departmentId_userId: { departmentId, userId: u.id } },
    create: { departmentId, userId: u.id },
    update: {},
  });
...
export async function DELETE(req: Request, ctx: { params: Promise<{ departmentId: string }> }) {
  ...
  await prisma.departmentInstructor.deleteMany({ where: { departmentId, userId } });
  return NextResponse.json({ ok: true });
}
