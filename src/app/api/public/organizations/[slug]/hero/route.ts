...
import { isSafeOrgHeroSettingStoredValue } from "@/lib/org/public-assets";
...
export async function GET(_req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  if (!org?.heroImageUrl?.trim()) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }
  const key = org.heroImageUrl.trim();
  if (/^https?:\/\//i.test(key)) {
  ...
  if (!isSafeOrgHeroSettingStoredValue(key, org.id)) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }
