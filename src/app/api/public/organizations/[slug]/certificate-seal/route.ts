...
import { isSafeOrgCertificateSealStoredValue } from "@/lib/org/public-assets";
...
export async function GET(_req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const key = parseOrganizationSettings(org.organizationSettings).certificateSealImageUrl?.trim();
  if (!key) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }
  ...
  if (!isSafeOrgCertificateSealStoredValue(key, org.id)) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }
