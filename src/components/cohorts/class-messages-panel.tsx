...
export function ClassMessagesPanel({
  cohortId,
  canPost,
}: {
  cohortId: string;
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/cohorts/${cohortId}/messages`, { credentials: "include" });
      ...
      {canPost ? (
        <div className="space-y-2">
          ...
          <Button type="button" disabled={sending || !body.trim()} onClick={() => void send()}>
            {sending ? "Sending…" : "Post to class"}
          </Button>
        </div>
      ) : (
        <p className="text-sm text-muted-foreground">
          Teachers and admins post announcements here. Students can read; use Messages to chat with classmates or
          teachers.
        </p>
      )}
      {loading ? (
