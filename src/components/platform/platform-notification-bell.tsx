...
export function PlatformNotificationBell() {
  ...
  const refreshList = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/platform/notifications?limit=25");
      if (!res.ok) return;
  ...
  useEffect(() => {
    void refreshList();
    const t = window.setInterval(() => void refreshList(), 60_000);
  ...
  useEffect(() => {
    ...
    const onDoc = (e: MouseEvent) => {
      if (panelRef.current && !panelRef.current.contains(e.target as Node)) {
        const btn = (e.target as HTMLElement).closest("[data-platform-notification-bell-trigger]");
        if (btn) return;
        setOpen(false);
      }
    };
  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        type="button"
        data-platform-notification-bell-trigger
        className={cn(
      ...
      {open ? (
        <div className="absolute right-0 z-50 mt-2 w-[min(100vw-2rem,22rem)] rounded-lg border border-border bg-popover shadow-lg">
          <div className="flex items-center justify-between border-b border-border px-3 py-2">
            ...
            {unreadCount > 0 ? (
              <button type="button" className="text-xs text-primary hover:underline" onClick={() => void markAll()}>
                Mark all read
              </button>
            ) : null}
          </div>
            ...
            {loading && rows.length === 0 ? (
              ...
              <ul className="divide-y divide-border">
                {rows.map((n) => {
                  ...
                  const href = n.link?.startsWith("/") ? n.link : null;
                  if (href) {
                    return (
                      <li key={n.id}>
                        <Link
